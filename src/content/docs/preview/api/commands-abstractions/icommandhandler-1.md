...
Represents a handler for commands implementing [`ICommand`](/ergosfare.docs/preview/api/commands-abstractions/icommand).
...
| Name | Description |
| --- | --- |
| `TCommand` | The type of command this handler processes. Must implement [`ICommand`](/ergosfare.docs/preview/api/commands-abstractions/icommand). |

## Remarks

This interface is non-generic regarding the result type; the command may or may not produce a result.
For commands with a strongly typed result, consider using [`ICommandHandler<TCommand, TResult>`](/ergosfare.docs/preview/api/commands-abstractions/icommandhandler-2).
