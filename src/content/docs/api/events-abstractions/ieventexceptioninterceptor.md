...
Handles failures raised while publishing an event that implements [`IEvent`](/ergosfare.docs/api/events-abstractions/ievent).

```csharp
public interface IEventExceptionInterceptor : IEvent, IMessage, IAsyncExceptionInterceptor<IEvent, Unit>, IExceptionInterceptor
```

[View source](https://github.com/stellayazilim/Ergosfare/blob/main/src/Stella.Ergosfare.Events.Abstractions/ExceptionInterceptors/IEventExceptionInterceptor.cs#L15)
...
[`IEventExceptionInterceptorFor<TException>`](/ergosfare.docs/api/events-abstractions/ieventexceptioninterceptorfor-1) to accept only certain failures.
...
| --- | --- | --- |
| `event` | [`IEvent`](/ergosfare.docs/api/events-abstractions/ievent) | The event whose publish failed. |
