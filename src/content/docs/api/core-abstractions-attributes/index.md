...
| [`ExcludeFromDiscoveryAttribute`](/ergosfare.docs/api/core-abstractions-attributes/excludefromdiscoveryattribute) | Class | Excludes a registrable construct from automatic discovery entirely: both source-generated registration and reflection-based assembly scanning (`RegisterFromAssembly`) skip the type, regardless of discovery keys or patterns. Explicit registration (`Register<T>()`, `Register(Type)`) still works. |
